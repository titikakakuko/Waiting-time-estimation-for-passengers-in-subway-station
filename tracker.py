...
def draw_bboxes(image, bboxes, time_counting, line_thickness):
    line_thickness = line_thickness or round(
        0.0002 * (image.shape[0] + image.shape[1]) * 0.5) + 1
    ...
    for (x1, y1, x2, y2, cls_id, pos_id) in bboxes:
        ...
        print('find id = ', pos_id)
        cv2.putText(image, 'ID-{} time:{}s'.format(pos_id, time_counting[pos_id]), (c1[0], c1[1] - 2), 0, line_thickness / 3,
